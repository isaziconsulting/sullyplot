...
Ensure there is never more than two of a plot type (e.g. 2 box plots, 2 histograms, 2 scatter plots)
...
Make sure your plots follow the available plot types & rules:
  ...
  Here are the available plot types & rules:
    ...
    box plots - must have a categorical on the x axis and a quantitative on the y axis. Can have multiple axes if related.
...
- \"descriptions\": A list of plot descriptions explaining each plot and how it must show the relevant relationship.
...
Most importantly, replace any plots that do not follow the plot rules:
  Here are the available plot types & rules:
    scatter plots - must have quantitatives on the x and y axes, and an optional 3rd categorical or quantitative column for colour.
    line plot - must have a DateTime column on the x-axis, a quantitative on the y axis, and an optional 3rd categorical column for colour.
    box plots - must have a categorical on the x axis and a quantitative on the y axis. Can have an optional 3rd categorical column for grouped box plots.
    histogram - must have a quantitative on the x-axis in bins, counts on the y axis, and an optional 2nd categorical column for stacked bars, multiple axes, or colour.
...
- 95%% prediction elipse per category for scatter plots that have a 3rd categorical column
- Lines of best fit with 5%% confidence intervals
- Multiple y axes for each category if there are too many categories
- Group related plots together and when you do, also add a completely new different plot so the total number of plots does not change - e.g. If there are two box/bar plots with the same categories use multiple y-axes to plot them together.
- Replace any redundant plots
...
# ** The default theme should be black axes, a white background with grey grid lines, and light blue for data, never plot in black**
# ** When you need mutliple colours use a colour palette of light blue, pink, light green, yellow, light red and orange for data visualisation **

fix_error_prompt <- "Your previous code: %s
    Failed with: %s
    Rewrite the function to account for this error and respond with only the new code and nothing else."
