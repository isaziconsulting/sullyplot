...
3. **Visual Enhancements**:
   - Where appropriate and not present already, add or improve existing visual enhancements:
     - Add categorical columns from the summary if they are logically relevant to show relationships across different categories. Explain how these should be used.
     ...
     - Group related plots together into one plot. For example, if there are two box/bar plots with the same categories, use multiple separate y-axes to plot them as a single `ggplot` (ensure not to include a large number of separate y-axes).
